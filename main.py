...
parser.add_argument('--save_schedule', default=[], nargs='*', type=int,
                    help='save checkpoint schedule (costom)')
parser.add_argument('--save_path', default='', type=str, metavar='PATH',
                    help='path to checkpoint filename (default: none)')
...
def main():
    ...
    test_loader = torch.utils.data.DataLoader(
        datasets.ImageFolderInstance(testdir, transforms.Compose([
            transforms.Resize(256),
            transforms.CenterCrop(224),
            transforms.ToTensor(),
            normalize,
    ...
    if args.evaluate:
        kNN(0, model, lemniscate, train_loader, test_loader, 25, args.nce_t, thresholding=True)
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        #The network also saves certain epochs......
        for epid in args.save_schedule:
            if epoch == epid:
                save_extra_checkpoint({
                    'arch': args.arch,
                    'state_dict': model.state_dict(),
                    'lemniscate': lemniscate,
                    'best_prec1': best_prec1,
...
def train(train_loader, model, lemniscate, criterion, optimizer, epoch):
    ...
    for i, (input, _, index) in enumerate(train_loader):
        ...
        loss = criterion(output, index)*0.1 / args.iter_size
...
def save_checkpoint(state, is_best, args, filename='checkpoint.pth.tar'):
    ...
    if is_best:
        best_filename = os.path.join(args.save_path,'model_best.pth.tar')
        shutil.copyfile(filename, best_filename)

def save_extra_checkpoint(state, is_best, args, epid):
    filename = 'checkpoint_'+str(epid)+'.pth.tar'
